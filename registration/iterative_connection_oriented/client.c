...
int main(int argc, char *argv[])
{
    ...
    {
        printf("INFO: (Client) Connected to server successfully\n");
    }
    ...
    // Send data to server
    sprintf(buffer, "%d,%s,%s,%s\n", student_info.serial_number, student_info.reg_number, student_info.first_name, student_info.last_name);
    ...
    if ((recv_size = recv(client_socket, buffer, BUFFER_SIZE, 0)) < 0)
    {
